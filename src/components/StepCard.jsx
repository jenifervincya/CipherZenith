...
import { STEP_EXPLAINERS } from "../data/stepExplainers";
 ...
 *
 * Clickable — expands into a technical explainer drawer showing what the
 * step actually does under the hood (static demo content, not live data).
 */
export default function StepCard({ index, title, status = "idle", details = {}, timestamp }) {
  ...
  const explainer = STEP_EXPLAINERS[index];

  return (
    <motion.div
      layout
      className={`relative overflow-hidden rounded-md bg-[#0b0f14] border border-white/5 ${style.ring} cursor-pointer`}
        ...
        {expanded && explainer && (
          ...
          >
            <div className="p-4 bg-white/[0.02]">
              <p className="text-xs text-cyan-400 font-medium mb-1.5">{explainer.tagline}</p>
              <p className="text-[11px] text-white/50 leading-relaxed mb-3">{explainer.body}</p>
              <p className="text-[9px] font-mono tracking-wider text-white/25 mb-1.5">
                UNDER THE HOOD
              </p>
              <ul className="space-y-1">
                {explainer.underTheHood.map((line, i) => (
